...
export default function SinglePrint({item,setItem}) {
   ...
   useEffect(()=>{
    ...
    return (
      ...
      <div id="_print" className={`w-full text-[0.9rem]  flex flex-col overflow-y-auto h-[100vh] ${!item ? '':'_print'}   fixed left-0 top-0 z-50 px-10 bg-white`} style={{zIndex:9999}}>
                 ...
                 <div className={`${item?.from=="medical-certificate" ? 'hidden':''}`}>
                        <div className="flex px-5 mt-5 justify-between">
                            ...
                            <div className="flex items-end ml-5 w-[170px]">
                                <span>Pro. No:</span>
                                <span className="flex border-b border-b-gray-600 ml-4 min-w-[200px]">{(item?.appointment?.dependent?.created_at || item?.patient?.created_at)?.split('T')?.[0]?.split('-')?.[0]}/{(item?.patient?.id || '').toString().padStart(4, '0')} {item?.appointment?.is_for_dependent ? ' - '+(item?.appointment?.dependent?.id || '').toString().padStart(3, '0') : ''}</span>
                            </div>
                        ...
                        <div className="flex px-5 mt-10 justify-between">
                            ...
                            <div className="flex items-end ml-5 w-[170px]">
                                <span>Tel:</span>
                                <span className="flex border-b border-b-gray-600 ml-4 min-w-[200px]">{item?.appointment?.dependent?.main_contact  || item?.patient?.main_contact}</span>
                            </div>
               ...
               <div className="w-full text-[0.8rem] text-gray-500 flex justify-between mt-10 border-t border-t-gray-200 pt-3">
                    <span>
                       <QRCodeGenerator code={(item?.i?.id || '').toString().padStart(5, '0')} link={`${data.APP_FRONDEND}/qrcode/${data.encodeBase64Multiple(`${(item?.i?.id || '').toString().padStart(5, '0')}---${item?.from}---${item?.appointment?.is_for_dependent ? item?.appointment?.dependent?.name  :  item?.patient?.name}---${item?.i?.created_at}`)}`}/>
                    </span>
