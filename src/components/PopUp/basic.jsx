...
function BasicPopUp({link,title,btnConfirm,btnCancel,message,show,preventClosing}) {
  const data=useData()
  return (
     <div id="info-popup" tabindex="-1" class={`overflow-y-auto ${preventClosing ? '_basic_popup':''} ${data._openPopUps.basic_popup=="you-have-saved-appointment"  ?  '_basic_popup':''} bg-[rgba(0,0,0,0.3)] ${!show ? 'opacity-0 translate-y-[60px] pointer-events-none':''} fixed top-0 right-0 items-center flex justify-center left-0 z-50 w-full  delay-0 ease-in transition-all md:inset-0 h-[100vh]`}>
