...
import { disableDates, getDateFormatMMDDYYYY, getDayElement, getMinHeight, getMonthAndYear, indicesToShow, LocalizedUtils, navigateMonth } from './utils.jsx';
...
const CalendarPicker = (props) => {
  ...
  const [firstSelectedDate, setFirstSelectedDateInside] = useState(new Date().toString());
  const [lastSelectedDate, setLastSelectedDateInside] = useState(new Date().toString());
    ...
    <React.Fragment>
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        >
          ...
          <span className={styles.endDateTxt}>
            {" "}
            {getDateFormatMMDDYYYY(lastSelectedDate || firstSelectedDate)}
          </span>
        </div>
      </div>
      <div
        id="navDate"
        style={{ display: "flex", flexDirection: "column", alignItems: "center", margin: "15px" }}
      >
      ...
      >
        <MuiPickersUtilsProvider utils={LocalizedUtils}>
          <ThemeProvider theme={materialThemeDateRangeSelector()}>
            <div>
              <DatePicker
                id="dateRangePicker"
                value={navMonth}
                onChange={(dt) => handleDateChange(new Date(dt))}
                variant="static"
                ...
                minDate={minDt}
                maxDate={maxDt}
                shouldDisableDate={(day) => disableDates(day, calendarData, new Date())}
              />
              <div className={styles.legendDiv}>
                ...
                {showBlackoutLegend ? (
                  <div className={styles.blackoutDiv}>
                    ...
                    <span className={styles.blackoutText}>Blackout Dates</span>
                  </div>
                ) : (
                  ""
                )}
                <div style={{ marginLeft: "8px", textAlign: "left" }}>
                  {indices && indices.length > 0
                    ? indices.map((it, key) => indicesToShow(it, key))
                    : ""}
                </div>
              </div>
            </div>
