...
function SelectDoctorAvailability({ item, setItem }) {
  ...
  return (
    <div style={{zIndex:item?.id ? 100 : -1}} className={`w-full h-[100vh] bg-[rgba(0,0,0,0.2)] ease-in _doctor_list ${!item?.id ? 'opacity-0 pointer-events-none translate-y-[100px]' : ''} ease-in transition-all delay-75 fixed flex items-center justify-center z-[99]`}>
      <div className="w-full overflow-y-auto p-4 relative bg-white border border-gray-200 rounded-lg shadow sm:p-8 z-40 max-w-[600px]">
        ...
        <a href="#" onClick={()=>{
                        ...
                        window.location.href=APP_FRONDEND+(`/login?nextpage=add-appointments&scheduled_doctor=${item.id}&scheduled_hours=${selectedDoctors[item.id]}&scheduled_date=${selectedDates[item.id] || new Date().toISOString().split('T')[0]}&scheduled_weekday=${weeks[new Date(selectedDates[item.id] || new Date().toISOString().split('T')).getDay()]}`)
