...
export default function BasicFilter({filterOptions,setFilterOptions,setUpdateFilters,show}) {
  ...
  return (
    <div  style={{width:data.showFilters ? '190px':'0'}} className={`min-h-[300px]  ${show ? 'opacity-0 pointer-events-none':'ease-in delay-75 transition-all'} overflow-x-hidden  relative mr-2`}>
          ...
          <div className="rounded-[2rem] pb-5 w-[170px] absolute left-0 top-0">
            ...
            {filterOptions.map((f,_f)=>(
                ...
                {f.items.filter(i=>(i.name || i.pt_name).toLowerCase().includes(f.search.toLowerCase())).map((i,_i)=>(
                    <li className="flex items-center px-[1px] cursor-pointer" onClick={() => {
                        ...
                        setFilterOptions(prev => {
                          ...
                          if (optionsIndex !== -1) {
                            newOptions[optionsIndex] = {
                              ...newOptions[optionsIndex],
                              selected_ids: newOptions[optionsIndex].selected_ids.includes(i.id)
                                ? newOptions[optionsIndex].selected_ids.filter(j => j !== i.id)
                                : [...newOptions[optionsIndex].selected_ids, i.id]
                            };
                          }
                          return newOptions;
                        });
                        ...
                        <input  checked={f.selected_ids.includes(i.id)} type="checkbox" value="" className="w-4 h-4 bg-gray-100 border-gray-300 rounded text-primary-600 focus:ring-primary-500 focus:ring-2" />
