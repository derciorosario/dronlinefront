...
function FindSpecialist() {
 ...
 const navigate=useNavigate()
  ...
  return (
    <div style={{zIndex:99}} className="w-full max-md:px-[20px] px-[50px] z-40 relative">
              ...
              <div className="w-full flex max-md:flex-col  max-md:gap-y-4 items-center gap-x-2">
                    ...
                    <div className="flex w-full items-center bg-white rounded-[0.3rem] p-2 relative">
                         ...
                         {selected2?.name && <div className="absolute px-2 rounded-[0.3rem] w-full h-full left-0 top-0 bg-white flex items-center justify-between">
                             <span>{selected2.name} ({data._specialty_categories.filter(i=>i.id==selected2.medical_specialty)[0]?.pt_name})</span>
                         ...
                         {data._openPopUps.show_doctors_list && <div className="w-full max-h-[300px] overflow-auto shadow z-40 absolute left-0 top-[100%] px-2 bg-white rounded-[0.3rem]">
                             ...
                             {searchedData2.filter(i=>i.medical_specialty==selected1?.id || !selected1).map(f=>(
                                ...
                                <div className="w-[50px] h-[50px] mr-3 rounded-full bg-slate-100 flex items-center justify-center">{f.name.charAt().toUpperCase()}</div>
                                    <div className="flex flex-col">
                                        <span className="mt-medium">{f.name}</span>
                                        <span>{data._specialty_categories.filter(i=>i.id==f.medical_specialty)[0]?.pt_name}</span>
                                    </div>
                                </div>
                             ))}
                    ...
                    }} className="px-5 py-[0.6rem] max-md:w-full whitespace-nowrap bg-honolulu_blue-300 text-white table uppercase text-[14px] border-honolulu_blue-300 border rounded-[0.3rem]">{t('menu.add-appointment')}</button>
