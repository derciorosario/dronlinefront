            ...
            <BaiscTable canAdd={user?.role=="admin"} addPath={'/add-doctors'} loaded={data._loaded.includes('doctors')  && !loading} header={[
                       ...
                       body={(data._doctors?.data || []).map((i,_i)=>(
                              ...
                              <BaiscTable.Tr>
                                ...
                                <BaiscTable.Td url={`/doctor/`+i.id}>{i.consultation_type=="individual-and-group"? t('common.individual-and-group') : i.consultation_type=="group" ? t('common.in-group') : t('common.individual')}</BaiscTable.Td>
                                ...
                                <BaiscTable.Td url={`/doctor/`+i.id}>{data._c_date(i.created_at).split('T')[0]?.split('-')?.reverse()?.join('/') + " "+data._c_date(i.created_at).split('T')[1].slice(0,5)}</BaiscTable.Td>
                                <BaiscTable.Td url={`/doctor/`+i.id}>{i.updated_at ? data._c_date(i.updated_at).split('T')[0]?.split('-')?.reverse()?.join('/') + " " +data._c_date(i.updated_at).split('T')[1].slice(0,5) : data._c_date(i.created_at).split('T')[0]?.split('-')?.reverse()?.join('/') + " "+data._c_date(i.created_at).split('T')[1].slice(0,5)}</BaiscTable.Td>
