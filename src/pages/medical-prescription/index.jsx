               ...
               <BaiscTable canAdd={false} loaded={data._loaded.includes('medical_prescriptions')} header={[
                          ...
                          body={(data._medical_prescriptions?.data || []).map((i,_i)=>(
                                 ...
                                 <BaiscTable.Tr >
                                   ...
                                   <BaiscTable.Td onClick={()=>globalOnclick(i.id)} minWidth={300}>{i.medical_prescription_items.map(i=>`${i.name ? `${_medications.filter(f=>f.ITEM==i.name)?.[0]?.name}` : i.custom_name} (${i.prescribed_quantity})`).join(', ')}</BaiscTable.Td>
