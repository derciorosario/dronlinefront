 ...
 useEffect(()=>{
  ...
  (async()=>{
    try{
     ...
     setForm({...form,...response,
      normal_consultation_price:data.formatNumber(data._cn_op(response.normal_consultation_price)),
      urgent_consultation_price:data.formatNumber(data._cn_op(response.urgent_consultation_price)),
      home_consultation_price:data.formatNumber(data._cn_op(response.home_consultation_price))
     })
     setLoading(false)
  ...
  async function SubmitForm(){
    ...
    try{
      ...
      let form_data={
        ...form,
        normal_consultation_price:data._cc(form.normal_consultation_price),
        urgent_consultation_price:data._cc(form.urgent_consultation_price),
        home_consultation_price:data._cc(form.home_consultation_price)
      }
      ...
      if(id){
        ...
        let r=await data.makeRequest({method:'post',url:`api/specialty-category/`+id,withToken:true,data:form_data, error: ``},0);
        ...
        let response=await data.makeRequest({method:'post',url:`api/specialty-categories/`,withToken:true,data:form_data, error: ``},0);
  ...
  useEffect(()=>{
    if(!user) return
    if(user?.role=="patient" || (user?.role=="manager" && !user?.data?.permissions?.specialty_categories?.includes('create') && !id)){
           navigate('/') 
    }
  },[user])
           ...
           <FormLayout  hide={!itemToEditLoaded && id} title={id ? t('common.edit-category') : t('common.add-category')} verified_inputs={verified_inputs} form={form}
                    ...
                    bottomContent={(
                        <div>
                            ...
                            {(form.id || !id) && <FileInput onlyImages={true} _upload={{key:'image_filename',filename:form.image_filename}} res={handleUploadedFiles} label={t('form.image')} r={true}/>}
                                <div className="w-[500px] flex items-center justify-center bg-gray-300 h-[150px] rounded-[0.3rem]">
                                   {!form.image_filename &&  <svg class="w-8 h-8 stroke-gray-400" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    {form.image_filename && <img className="object-cover border w-auto h-full" src={form.image_filename}/>}
                                </div>
                             </div>
                    ...
                    />
                     ...
                     <FormLayout.Input 
                        verified_inputs={verified_inputs} 
                        form={form}
                        onBlur={() => setVerifiedInputs([...verified_inputs, 'normal_consultation_price'])} 
                        label={t('common.normal_consultation_price')} 
                        onChange={(e) => setForm({...form, normal_consultation_price: data.formatNumber(data._cn_op(e.target.value))})} 
                        field={'normal_consultation_price'} 
                        value={form.normal_consultation_price}
                        r={true}
                    />
                    ...
                    <FormLayout.Input 
                        verified_inputs={verified_inputs} 
                        form={form}
                        onBlur={() => setVerifiedInputs([...verified_inputs, 'urgent_consultation_price'])} 
                        label={t('common.urgent_consultation_price')} 
                        onChange={(e) => setForm({...form, urgent_consultation_price: data.formatNumber(data._cn_op(e.target.value))})} 
                        field={'urgent_consultation_price'} 
                        value={form.urgent_consultation_price}
                        r={true}
                    />
                    ...
                    <FormLayout.Input 
                        verified_inputs={verified_inputs} 
                        form={form}
                        onBlur={() => setVerifiedInputs([...verified_inputs, 'home_consultation_price'])} 
                        label={t('common.home_consultation_price')} 
                        onChange={(e) => setForm({...form, home_consultation_price: data.formatNumber(data._cn_op(e.target.value))})} 
                        field={'home_consultation_price'} 
                        value={form.home_consultation_price}
                        r={true}
                    />
