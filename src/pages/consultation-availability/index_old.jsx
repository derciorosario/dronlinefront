                                ...
                                let date=new Date(selectedDate?.$d).toISOString().split('T')[0]
                                 ...
                                 return (
                                    (
                                        ...
                                        }} className={` ${!specificList.includes(date) && dateView=="calendar" ? 'opacity-40 pointer-events-none':''} inline-flex min-w-[100px] justify-center px-3 py-1 ${dateView!="calendar" ? (days[selectedWeek].includes(f) ? 'border-honolulu_blue-300 bg-honolulu_blue-50 text-honolulu_blue-400':urgent_days[selectedWeek].includes(f) ? ' border-orange-400 bg-orange-50  text-orange-400':'bg-gray-200 border-transparent')             :          (availableSpecific[date]?.includes(f) ? 'border-honolulu_blue-300 bg-honolulu_blue-50 text-honolulu_blue-400':urgentAvailableSpecific[date]?.includes(f) ? ' border-orange-400 bg-orange-50  text-orange-400':'bg-gray-200 border-transparent')} border rounded-full text-[14px] cursor-pointer`}>{f}</span>
                                   )
                                 )
                               })}
                          </div>
