...
function App() { 
  ...
  let required_data=['appointment_feedback']
 ...
 async function handleItems({status,id}){
  ...
  try{
   ...
   await data.makeRequest({method:'post',url:`api/reviews/${id}/status`,withToken:true,data:{
     status
   }, error: ``},0);
...
useEffect(()=>{
  if(!user) return
  if(user?.role=="manager" && !user?.data?.permissions?.feedback?.includes('read')){
         navigate('/') 
  }
},[user])
                ...
                body={(data._appointment_feedback?.reviews?.data || []).filter(i=>i.status==selectedTab || (!i.status && selectedTab=="pending")).map((i,_i)=>(
                       <BaiscTable.Tr>
                         ...
                         <BaiscTable.Td>
                          ...
                          <BaiscTable.Td >{i.updated_at ? data._c_date(i.updated_at).split('T')[0]?.split('-')?.reverse()?.join('/') + " " +data._c_date(i.updated_at).split('T')[1].slice(0,5) : data._c_date(i.created_at).split('T')[0]?.split('-')?.reverse()?.join('/') + " "+data._c_date(i.created_at).split('T')[1].slice(0,5)}</BaiscTable.Td>
